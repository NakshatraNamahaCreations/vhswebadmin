...
class appsubcat {
  ...
  async updateCategoryimgLink(req, res) {
    try {
      const subcategoryId = req.params.ccid;
      const { imglink } = req.body;

      const findsubCategory = await appresubcatModel.findOne({
        _id: subcategoryId,
      });
      if (!findsubCategory) {
        return res.json({ error: "No such record found" });
      ...
      findsubCategory.imglink = imglink || findsubCategory.imglink;

      const updateCategory = await appresubcatModel.findOneAndUpdate(
        { _id: subcategoryId },
        findsubCategory,
        { new: true }
      );
