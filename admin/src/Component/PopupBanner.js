...
export default function PopUpBanner() {
  ...
  const handleSaveOrUpdate = async () => {
    try {
      const url = `https://api.vijayhomeservice.com/api/pbanner/updatepopupbanner/${
  ...
  const handleAddCreations = async () => {
    try {
      const config = {
        url: "https://api.vijayhomeservice.com/api/pbanner/addpopupbanner",
  ...
  const handleDelete = async (row) => {
    try {
      const res = await axios.post(
        `https://api.vijayhomeservice.com/api/pbanner/deletepopupbanner/${row._id}`
      );
      if (res.status === 200) {
    ...
    <div className="row m-auto p-2">
      <div className="row text-center">
        <div className="col-md-3 d-flex m-auto">
          <span className="m-auto text-bold"> Pop Up Banner Management </span>
          <MdOutlineLibraryAdd
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            {Edit ? "Edit Pop Up Banner " : "Add Pop Up Banner "}{" "}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
